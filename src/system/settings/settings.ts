...
const SHOP_OVERLAY_OPACITY_OPTIONS: SettingOption[] = new Array(9)
  .fill(null)
  .map((_, i) => {
...
export function setSetting(setting: string, value: number): boolean {
  ...
  switch (Setting[index].key) {
    case SettingKeys.Game_Speed:
      globalScene.gameSpeed = parseFloat(
    ...
    case SettingKeys.Player_Gender:
      if (globalScene.gameData) {
        ...
        globalScene.trainer.setTexture(
          globalScene.trainer.texture.key.replace("m", "m")
        );
      } else {
        return false;
      }
      ...
      if (value) {
        if (globalScene.ui) {
          const cancelHandler = () => {
            ...
            (globalScene.ui.getHandler() as SettingsUiHandler).setOptionCursor(
              -1,
              0,
              true
            );
