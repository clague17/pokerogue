...
export class SelectModifierPhase extends BattlePhase {
  ...
  start() {
    ...
    if (!!this.customModifierSettings) {
      const newItemCount =
        ...
        (this.customModifierSettings.guaranteedModifierTypeOptions?.length ||
          0) +
      ...
      if (this.customModifierSettings.fillRemaining) {
        ...
        modifierCount.value =
          originalCount > newItemCount ? originalCount : newItemCount;
      } else {
    ...
    const modifierSelectCallback = (rowCursor: number, cursor: number) => {
        ...
        case 0:
          switch (cursor) {
            ...
            case 3:
              ...
              const uiHandler =
                globalScene.ui.getHandler() as ModifierSelectUiHandler;
      ...
      if (
        ...
        !Overrides.WAIVE_ROLL_FEE_OVERRIDE
      ) {
  ...
  getRerollCost(lockRarities: boolean): number {
    ...
    const baseMultiplier = Math.min(
      Math.ceil(globalScene.currentBattle.waveIndex / 10) *
        baseValue *
        2 ** this.rerollCount *
        multiplier,
