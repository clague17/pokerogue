...
import type {
  OptionSelectConfig,
  OptionSelectItem,
} from "#app/ui/abstact-option-select-ui-handler";
...
export class TitlePhase extends Phase {
  ...
  showOptions(): void {
    ...
    if (loggedInUser && loggedInUser.lastSessionSlot > -1) {
      options.push({
        ...
        handler: () => {
          this.loadSaveSlot(
            this.lastSessionData || !loggedInUser
              ? -1
  ...
  loadSaveSlot(slotId: number): void {
    globalScene.sessionSlotId =
      slotId > -1 || !loggedInUser ? slotId : loggedInUser.lastSessionSlot;
    globalScene.ui.setMode(Mode.MESSAGE);
  ...
  initDailyRun(): void {
      ...
      (slotId: number) => {
        ...
        const generateDaily = (seed: string) => {
          ...
          for (const starter of starters) {
            const starterProps = globalScene.gameData.getSpeciesDexAttrProps(
              starter.species,
              starter.dexAttr
            );
          ...
          const modifiers: Modifier[] = Array(3)
            ...
            .concat([
              modifierTypes
                .MAP()
                .withIdFromFunc(modifierTypes.MAP)
                .newModifier(),
            ])
    ...
    if (!this.loaded && !globalScene.gameMode.isDaily) {
      ...
      // Start the overworld scene instead of continuing with battle phases
      globalScene.ui.fadeOut(500).then(() => {
        globalScene.scene.start("overworld");
      });
      ...
      if (
        ...
        if (availablePartyMembers > minPartySize) {
          ...
          if (globalScene.currentBattle.double) {
            globalScene.pushPhase(
              new CheckSwitchPhase(1, globalScene.currentBattle.double)
            );
          }
        }
      }
