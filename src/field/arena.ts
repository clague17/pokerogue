...
export class Arena {
  ...
  randomTrainerType(waveIndex: number, isBoss: boolean = false): TrainerType {
    const isTrainerBoss =
      ...
      (globalScene.gameMode.isTrainerBoss(
        waveIndex,
        this.biomeType,
        globalScene.offsetGym
      ) ||
        isBoss);
  ...
  triggerWeatherBasedFormChangesToNormal(): void {
    globalScene.getField(true).forEach((p) => {
      const isCastformWithForecast =
        p.hasAbility(Abilities.FORECAST, false, true) &&
        p.species.speciesId === Species.CASTFORM;
      const isCherrimWithFlowerGift =
        p.hasAbility(Abilities.FLOWER_GIFT, false, true) &&
        p.species.speciesId === Species.CHERRIM;

      if (isCastformWithForecast || isCherrimWithFlowerGift) {
        new ShowAbilityPhase(p.getBattlerIndex());
        return globalScene.triggerPokemonFormChange(
          p,
          SpeciesFormChangeRevertWeatherFormTrigger
        );
      }
  ...
  public isMoveWeatherCancelled(user: Pokemon, move: Move): boolean {
    return (
      !!this.weather &&
      !this.weather.isEffectSuppressed() &&
      this.weather.isMoveWeatherCancelled(user, move)
    );
  }

  public isMoveTerrainCancelled(
    user: Pokemon,
  ...
  addTag(
    ...
    if (newTag) {
      ...
      const { layers = 0, maxLayers = 0 } =
        newTag instanceof ArenaTrapTag ? newTag : {};
  ...
  lapseTags(): void {
    this.tags
      ...
      .forEach((t) => {
        ...
        this.eventTarget.dispatchEvent(
          new TagRemovedEvent(t.tagType, t.side, t.turnCount)
        );
      });
  }
  ...
  removeTag(tagType: ArenaTagType): boolean {
    ...
    if (tag) {
      ...
      this.eventTarget.dispatchEvent(
        new TagRemovedEvent(tag.tagType, tag.side, tag.turnCount)
      );
    }
    return !!tag;
  }
  ...
  getBgmLoopPoint(): number {
    switch (this.biomeType) {
      ...
      default:
        console.warn(
          `missing bgm loop-point for biome "${Biome[this.biomeType]}" (=${
            this.biomeType
          })`
        );
        return 0;
        ...
        () => {
          ...
          this.props.forEach((prop, p) => {
            ...
            if (hasProps && prop.texture.frameTotal > 1) {
              const propFrameNames = globalScene.anims.generateFrameNames(
                propKey,
                {
                  zeroPad: 4,
                  suffix: ".png",
