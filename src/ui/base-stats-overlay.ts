...
export class BaseStatsOverlay
  extends Phaser.GameObjects.Container
  implements InfoToggle {
  ...
  constructor(options?: BaseStatsOverlaySettings) {
    ...
    this.width =
      (options?.width || BaseStatsOverlay.getWidth(this.scale)) / this.scale; // divide by scale as we always want this to be half a window wide
    this.statsBg = addWindow(0, 0, this.width, HEIGHT);
    this.statsBg.setOrigin(0, 0);
    this.add(this.statsBg);

    for (let i = 0; i < 6; i++) {
      const shadow = globalScene.add.rectangle(
        this.width - BORDER + 1,
        BORDER + 3 + i * 15,
        100,
        5,
      ...
      const rectangle = globalScene.add.rectangle(
        this.width - BORDER,
        BORDER + 2 + i * 15,
        100,
        5,
    ...
    this.statsTotalLabel = addTextObject(
      BORDER,
      BORDER + 6 * 15,
      "A",
  ...
  show(values: number[], total: number): boolean {
    for (let i = 0; i < 6; i++) {
      this.statsLabels[i].setText(
        i18next.t(`pokemonInfo:Stat.${shortStats[i]}shortened`) +
          ": " +
          `${values[i]}`
      );
    ...
    this.statsTotalLabel.setText(
      i18next.t("pokedexUiHandler:baseTotal") + ": " + `${total}`
    );
