...
export default class ModifierSelectUiHandler extends AwaitableUiHandler {
  ...
  setup() {
    ...
    if (context) {
      ...
      this.transferButtonWidth = context.measureText(
        i18next.t("modifierSelectUiHandler:transfer")
      ).width;
      this.checkButtonWidth = context.measureText(
    ...
    this.rerollButtonContainer = globalScene.add.container(
      16,
      OPTION_BUTTON_YPOSITION
    );
    ...
    this.lockRarityButtonContainer = globalScene.add.container(
      16,
      OPTION_BUTTON_YPOSITION
    );
  ...
  show(args: any[]): boolean {
    ...
    const canLockRarities = !!globalScene.findModifier(
    ...
    const optionsYOffset =
      shopTypeOptions.length > SHOP_OPTIONS_ROW_LIMIT
        ? -SINGLE_SHOP_ROW_YOFFSET
        : -DOUBLE_SHOP_ROW_YOFFSET;

    for (let m = 0; m < typeOptions.length; m++) {
      const sliceWidth =
        globalScene.game.canvas.width / 6 / (typeOptions.length + 2);
    ...
    const continueButton =
      this.continueButtonContainer.getAt<Phaser.GameObjects.Text>(0);
    ...
    for (let m = 0; m < shopTypeOptions.length; m++) {
      ...
      const sliceWidth =
        globalScene.game.canvas.width / 6 / (rowOptions.length + 2);
      const option = new ModifierOption(
    ...
    globalScene.time.delayedCall(4000 + maxUpgradeCount * 2000, () => {
      ...
      const updateCursorTarget = () => {
        if (globalScene.shopCursorTarget === ShopCursorTarget.CHECK_TEAM) {
          ...
          globalScene.gameMode.hasNoShop
        ) {
            ...
            switch (this.cursor) {
              ...
              case 1:
                if (this.lockRarityButtonContainer.visible) {
                  ...
                  success =
                    this.rerollButtonContainer.visible && this.setCursor(0);
                }
                break;
              case 2:
    ...
    if (this.rowCursor) {
      if (this.rowCursor === 1 && options.length === 0) {
        ...
        this.cursorObj.setPosition(
          ...
          -globalScene.game.canvas.height / 12 -
            (this.shopOptionsRows.length > 1
              ? SINGLE_SHOP_ROW_YOFFSET - 2
              : DOUBLE_SHOP_ROW_YOFFSET - 2)
        );
        ui.showText(
          i18next.t("modifierSelectUiHandler:continueNextWaveDescription")
        );
        return ret;
      }

      const sliceWidth =
        globalScene.game.canvas.width / 6 / (options.length + 2);
      if (this.rowCursor < 2) {
  ...
  updateRerollCostText(): void {
    ...
    const formattedMoney = Utils.formatMoney(
      globalScene.moneyFormat,
      this.rerollCost
    );

    this.rerollCostText.setText(
      i18next.t("modifierSelectUiHandler:rerollCost", { formattedMoney })
    );
  ...
  updateLockRaritiesText(): void {
    const textStyle = globalScene.lockModifierTiers
...
class ModifierOption extends Phaser.GameObjects.Container {
  ...
  setup() {
    if (!this.modifierTypeOption.cost) {
      const getPb = (): Phaser.GameObjects.Sprite => {
        const pb = globalScene.add.sprite(
          0,
          -182,
          "pb",
  ...
  getPbAtlasKey(tierOffset: number = 0) {
    return getPokeballAtlasKey(
      (this.modifierTypeOption.type?.tier! +
        tierOffset) as number as PokeballType
    ); // TODO: is this bang correct?
  }

  updateCostText(): void {
    const cost = Overrides.WAIVE_ROLL_FEE_OVERRIDE
      ? 0
      : this.modifierTypeOption.cost;
    const textStyle =
      cost <= globalScene.money ? TextStyle.MONEY : TextStyle.PARTY_RED;

    const formattedMoney = Utils.formatMoney(globalScene.moneyFormat, cost);

    this.itemCostText.setText(
      i18next.t("modifierSelectUiHandler:itemCost", { formattedMoney })
    );
    this.itemCostText.setColor(
      getTextColor(textStyle, false, globalScene.uiTheme)
    );
    this.itemCostText.setShadowColor(
      getTextColor(textStyle, true, globalScene.uiTheme)
    );
  }
}
